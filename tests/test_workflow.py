...
def test_full_workflow_retries_interrupts_and_resumes(test_settings):
    ...
    paused = app.workflow.invoke(request.model_dump(mode="json"), config=config)
    approval_payload = _interrupt_value(paused)
    assert approval_payload["type"] == "human_approval"
    assert "VPN" in " ".join(approval_payload["risk_flags"])
    ...
    completed = app.workflow.invoke(Command(resume=approval.model_dump(mode="json")), config=config)

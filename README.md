...
4. If only the notebook is open, the bootstrap cell asks for `OnboardAI_Final_Capstone.zip`.
   Upload it once; the cell extracts the project and runs `pip install -e ".[dev]"`.
...
For live evidence, set `GROQ_API_KEY` and `LANGSMITH_API_KEY` in the environment rather than in
source code.

## Git practices

Use meaningful incremental commits when you push the final repository. A good sequence is:

1. `docs: initialize OnboardAI capstone and architecture`
2. `feat: add approved HR knowledge base and hybrid RAG`
3. `feat: add Track A supervisor and specialist agents`
4. `feat: add functional workflow persistence and human approval`
5. `feat: add reliability grounding and cross-thread memory evidence`
6. `docs: add rubric mapping and executed capstone notebook`

Do not commit `.env`, runtime databases, generated drafts, Hugging Face caches, or API keys.
...
## Course pages used as the implementation target

- Capstone rubric: https://mohammadyusif.github.io/agentic-ai-systems/L01/capstone_prep.html
- RAG agent: https://mohammadyusif.github.io/agentic-ai-systems/L01/06_RAG_agent.html
- Supervisor/handoffs: https://mohammadyusif.github.io/agentic-ai-systems/L01/08b_supervisor_and_handoffs.html
- Workflows: https://mohammadyusif.github.io/agentic-ai-systems/L01/10_workflows.html
- Long-term memory: https://mohammadyusif.github.io/agentic-ai-systems/L01/12_long_term_memory.html
- Reliability: https://mohammadyusif.github.io/agentic-ai-systems/L01/13_reliability.html


## Final V2 retrieval reliability fix

Live specialist workers now use a two-phase LangChain interaction:

1. a tool-bound LLM call is required to emit the semantic retrieval call and generates its query/categories;
2. the returned evidence is then passed to `with_structured_output(WorkerResult)`.

This prevents provider structured-output behavior from skipping the RAG tool while preserving
visible, genuine LLM tool-calling evidence.

"""Composition root for live and offline capstone modes."""
...
class OnboardAIApp:
    ...
    def close(self) -> None:
        ...
        self.services.operations.close()
...
def reset_runtime(settings: Settings | None = None) -> None:
    """Remove generated demo state so the final notebook can produce one clean run."""

    settings = settings or Settings.from_env(require_live_key=False)
    for path in (settings.checkpoint_path, settings.operations_path):
        if path.exists():
            path.unlink()

    if settings.drafts_dir.exists():
        for path in settings.drafts_dir.glob("*"):
            if path.is_file():
                path.unlink()
...
def build_app(
    *,
    ...
    embedding_label = (
        ...
        else "DeterministicHashEmbeddings (test double)"
    )
    ...
    if live:
        ...
        supervisor = StructuredSupervisor(model)
    ...
    services = AppServices(
        ...
        simulate_it_failure=(
            settings.simulate_it_failure
            if simulate_it_failure is None
            else simulate_it_failure
        ),
    )

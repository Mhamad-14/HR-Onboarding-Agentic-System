"""Environment-only configuration. Secrets are never stored in source code."""
...
class Settings:
    ...
    def from_env(cls, *, require_live_key: bool = False) -> "Settings":
        ...
        if require_live_key and not os.getenv("GROQ_API_KEY"):
            raise RuntimeError(
                "GROQ_API_KEY is missing. Use Colab Secrets or an environment variable; "
        ...
        settings = cls(
            ...
            simulate_it_failure=(
                os.getenv("ONBOARDAI_SIMULATE_IT_FAILURE", "false").lower() == "true"
            ),
        )
...
def langsmith_status() -> dict[str, bool | str]:
    return {
        ...
        "modern_tracing_flag": os.getenv("LANGSMITH_TRACING", "").lower() == "true",
        "project": (
            os.getenv("LANGSMITH_PROJECT")
            or os.getenv("LANGCHAIN_PROJECT")
            or "onboardai-final-capstone"
        ),
    }

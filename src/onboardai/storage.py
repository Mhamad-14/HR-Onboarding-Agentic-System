"""Sandboxed SQLite side effects and separate cross-thread employee Store."""
...
class TransientITError(ConnectionError):
    """Known retryable failure used to demonstrate LangGraph RetryPolicy."""
...
class EmployeeMemory:
    """Separate Store for safe facts that must survive a change of thread_id."""
    ...
    def recall_completed_training(self, employee_id: str) -> list[str]:
        item = self.store.get(self.namespace(employee_id), "completed_training")
        if not item:
            return []
        return list(item.value.get("courses", []))

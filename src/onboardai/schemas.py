"""Pydantic contracts shared by the boundary, supervisor, workers, and approval step."""
...
class OnboardingRequest(StrictModel):
    ...
    def reject_placeholder_names(cls, value: str) -> str:
        if value.lower() in {"test", "todo", "name", "employee"}:
            raise ValueError("Use a realistic synthetic name, not a placeholder")
        return value
...
class SupervisorDecision(StrictModel):
    destination: RouteDestination = Field(
        description=(
            "Select one unfinished requested worker. Use human_review only when a risk "
            "requires approval; use complete only after all requested workers finished."
        )
    )
...
class OnboardingOutcome(StrictModel):
    status: Literal["completed", "rejected", "needs_information"]
    case_id: str
    employee_id: str
    routing_log: list[SupervisorDecision]

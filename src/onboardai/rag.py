"""Hybrid RAG indexing and retrieval for approved onboarding knowledge."""
...
class DeterministicHashEmbeddings(Embeddings):
    """Small offline test double. Never present this as the submitted embedding model."""
...
class KnowledgeBase:
    ...
    def __init__(self, vector_store: InMemoryVectorStore, chunks: list[Document]):
    ...
    def build(
        ...
        *,
        ...
        return cls(vector_store=vector_store, chunks=chunks)
    ...
    def search(
        ...
        *,
        ...
        # Retrieve extra candidates before applying the metadata filter. This keeps the
        # vector-store implementation swappable while maintaining explicit source control.
        candidates = self.vector_store.similarity_search(query, k=max(k * 3, k))
        if categories:
    ...
    def evidence_report(self, query: str, *, k: int = 4) -> dict:
        citations = self.search(query, k=k)
        return {
            "query": query,
            "indexed_documents": len({doc.metadata.get("source") for doc in self.chunks}),
...
def citations_as_context(citations: list[Citation]) -> str:
    """Clearly delimit retrieved text so it is treated as data, not instructions."""
